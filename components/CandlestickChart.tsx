    ...
    if (trades.length > 0) {
      ...
      for (const trade of trades) {
        ...
        entryMarkerSeries.setData([
          ...
          // Use actual exit price if trade is closed and hit TP/SL, else use configured
          const actualExit = trade.exitTime && trade.exitPrice ? trade.exitPrice : null;
          let tpLevel: number, slLevel: number;
          if (trade.direction === 'long') {
            tpLevel = (trade.exitReason === 'take_profit' && actualExit) ? actualExit : trade.entryPrice * (1 + trade.takeProfit/100);
            slLevel = (trade.exitReason === 'stop_loss' && actualExit) ? actualExit : trade.entryPrice * (1 - trade.stopLoss/100);
          } else {
            tpLevel = (trade.exitReason === 'take_profit' && actualExit) ? actualExit : trade.entryPrice * (1 - trade.takeProfit/100);
            slLevel = (trade.exitReason === 'stop_loss' && actualExit) ? actualExit : trade.entryPrice * (1 + trade.stopLoss/100);
          }

          // Box for TP area (green)
          ...
          tpSeries.setData([
            ...
            { time: startTime, value: tpLevel },
            { time: endTime, value: tpLevel },
